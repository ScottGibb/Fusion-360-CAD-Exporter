...
def _capture_model_image(
    ...
    """Capture the model with modelling aids hidden, then restore the view.

    Folder switches are component-specific, so visit every component (including
    nested and referenced components), not just the root. Decals and body
    visibility are deliberately retained as part of the model's appearance.
    """
    viewport = app.activeViewport
    saved_camera = viewport.camera
    visibility = []
    selections = app.userInterface.activeSelections
    selected_entities = [selections.item(i).entity for i in range(selections.count)]
    selections_cleared = False

    def hide(target, property_name):
        previous = getattr(target, property_name)
        if previous:
            # Record before changing so even a failing setter can be restored.
            visibility.append((target, property_name, previous))
            setattr(target, property_name, False)

    try:
        grid_command = app.userInterface.commandDefinitions.itemById(
            "ViewLayoutGridCommand"
        )
        if grid_command:
            grid_control = adsk.core.ListControlDefinition.cast(
                grid_command.controlDefinition
            )
            if grid_control and grid_control.listItems.count:
                hide(grid_control.listItems.item(0), "isSelected")
        ...
        for component in design.allComponents:
            for property_name in (
                ...
                hide(component, property_name)
        hide(design.analyses, "isLightBulbOn")
        selections.clear()
        selections_cleared = True
        viewport.fit()
        viewport.refresh()
        ...
        if not viewport.saveAsImageFileWithOptions(image_options):
            raise RuntimeError("Failed to save viewport image.")
    finally:
        capture_failed = sys.exc_info()[0] is not None
        restore_errors = []
        # One failed restoration must not prevent the remaining view settings
        # from being restored, or mask the original capture error.
        for target, property_name, previous in reversed(visibility):
            try:
                setattr(target, property_name, previous)
            except Exception as error:
                restore_errors.append(f"{property_name}: {error}")
        try:
            viewport.camera = saved_camera
        except Exception as error:
            restore_errors.append(f"camera: {error}")
        for entity in selected_entities if selections_cleared else []:
            try:
                if not selections.add(entity):
                    restore_errors.append("selection could not be restored")
            except Exception as error:
                restore_errors.append(f"selection: {error}")
        try:
            viewport.refresh()
        except Exception as error:
            restore_errors.append(f"viewport: {error}")
        if restore_errors:
            message = "Could not fully restore the view: " + "; ".join(restore_errors)
            app.log(message)
